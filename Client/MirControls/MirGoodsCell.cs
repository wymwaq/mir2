...
namespace Client.MirControls
{
    public sealed class MirGoodsCell : MirControl
    {
        ...
        private void Update()
        {
            ...
            if (UsePearls)
            {
                PriceLabel.Text = string.Format($"{WqCr.PriceText}: {(uint)(Item.Price() * GameScene.NPCRate)} pearl{(Item.Price() > 1 ? "s" : "")}");
            }
            else if (Recipe)
            {
                ClientRecipeInfo recipe = GameScene.RecipeInfoList.SingleOrDefault(x => x.Item.ItemIndex == Item.ItemIndex);

                PriceLabel.Text = string.Format($"{WqCr.PriceText}: {(uint)(recipe.Gold * GameScene.NPCRate)} {WqCr.GoldText}");
            }
            else
            {
                PriceLabel.Text = string.Format($"{WqCr.PriceText}: {(uint)(Item.Price() * GameScene.NPCRate)} {WqCr.GoldText}");
            }
        }
