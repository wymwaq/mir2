        ...
        private void Confirm()
        {
            ...
            switch (PType)
            {
                case PanelType.Sell:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(WqCr.CannotSellItem, ChatType.System);
                        return;
                    }
                    ...
                    GameScene.Scene.ChatDialog.ReceiveChat(WqCr.CannotCarryAnymoreGold, ChatType.System);
                    break;
                case PanelType.Repair:
                    if (TargetItem.Info.Bind.HasFlag(BindMode.DontRepair))
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(WqCr.CannotRepairItem, ChatType.System);
                        return;
                    }
                ...
                case PanelType.Consign:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(WqCr.CannotConsignItem, ChatType.System);
                        return;
                    }
                    MirAmountBox box = new MirAmountBox($"{WqCr.ConsignmentPriceText}:", TargetItem.Image, Globals.MaxConsignment, Globals.MinConsignment)
                    {
                ...
                case PanelType.Refine:
                    ...
                    for (int i = 0; i < GameScene.Scene.RefineDialog.Grid.Length; i++)
                    {
                        if (GameScene.Scene.RefineDialog.Grid[i].Item != null)
                        {
                            ...
                            GameScene.Scene.ChatDialog.ReceiveChat(String.Format(WqCr.CannotConsignItem, TargetItem.FriendlyName), ChatType.System);
                            return;
                        }

                    }
                    GameScene.Scene.ChatDialog.ReceiveChat(String.Format(WqCr.DontHaveGoldRefine, TargetItem.FriendlyName), ChatType.System);
                    break;
                case PanelType.CheckRefine:

                    if (TargetItem.RefineAdded == 0)
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(String.Format(WqCr.HaventDepositedRefine, TargetItem.FriendlyName), ChatType.System);
                        return;
                    }
                ...
                case PanelType.ReplaceWedRing:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(String.Format(WqCr.IsntRing, TargetItem.FriendlyName), ChatType.System);
                        return;
                    }
        ...
        private void NPCDropPanel_BeforeDraw(object sender, EventArgs e)
        {
            ...
            switch (PType)
            {
                case PanelType.Sell:
                    ...
                    break;
                case PanelType.Repair:
                    text = $"{WqCr.RepairText}: ";
                    break;
                case PanelType.SpecialRepair:
                    text = $"{WqCr.SRepairText}: ";
                    break;
                case PanelType.Consign:
                    InfoLabel.Text = $"{WqCr.ConsignmentText}: ";
                    return;
                case PanelType.Disassemble:
                    text = $"{WqCr.ItemWillBeDestroyed}\n\n\n\n\n\n\n\n         ";
                ...
                case PanelType.Reset:
                    ...
                    break;
                case PanelType.Refine:
                    text = $"{WqCr.RefineText}: ";
                    HoldButton.Visible = false;
                    ConfirmButton.Visible = true;
                    GameScene.Scene.RefineDialog.Show();
                    break;
                case PanelType.CheckRefine:
                    text = $"{WqCr.CheckRefineText}";
                    HoldButton.Visible = false;
                    ConfirmButton.Visible = true;
                    break;
                case PanelType.ReplaceWedRing:
                    text = $"{WqCr.ReplaceText}: ";
                    HoldButton.Visible = false;
                    ConfirmButton.Visible = true;
                    break;
